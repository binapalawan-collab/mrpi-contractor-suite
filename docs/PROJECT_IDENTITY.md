# Project Identity Allowlist

This repository is a brand-new system. Legacy MRPI systems are out of scope and must not be inspected, migrated, modified, paused, deleted, or reused.

## Approved resources

- GitHub repository: `binapalawan-collab/mrpi-contractor-suite`
- GitHub URL: `https://github.com/binapalawan-collab/mrpi-contractor-suite`
- Supabase project name: `mrpi-contractor-suite-dev`
- Supabase project ref: `jhfvxsblckejvfnkjsrk`
- Supabase dashboard: `https://supabase.com/dashboard/project/jhfvxsblckejvfnkjsrk`
- Supabase region: `ap-southeast-1` (Singapore)
- Cloudflare Pages project: not created yet

Only resources listed here may be used for this application. Never commit passwords, secret keys, service-role keys, access tokens, or production credentials.

