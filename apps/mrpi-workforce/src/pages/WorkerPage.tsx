import { ArrowRight, FileText, Pencil, Plus, UserRoundCheck } from 'lucide-react'
  ...
  return <>
    ...
    {loading
      ...
      : !rows.length
        ...
        : <div className="grid gap-3 md:grid-cols-2">{rows.map((worker) => <article key={worker.id} className="card flex items-start gap-4 p-5">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-sky-50 text-sky-700"><UserRoundCheck className="h-5 w-5" /></span>
          <div className="min-w-0 flex-1">
            <div className="flex justify-between gap-3">
              <div><h2 className="font-black">{worker.name}</h2><p className="mt-1 text-sm text-slate-500">{payTypeLabel(worker.pay_type)}{worker.pay_type === 'daily' ? ` · ${formatMoney(worker.default_daily_rate)}/hari` : ''}</p></div>
